...
# set directories
setwd("N:\\Stock assessment\\Haddock WKNSMSE 2018\\WK_WKNSMSE_had.27.46a20 - final 20190315")
...
if(doPlot){
  ggplot(data=FLQuants(catch=catch(stk), landings=landings(stk), discards=discards(stk),
                       BMS=landings(stk.bms),IBC=landings(stk.ibc)), aes(year, data)) + 
    geom_line(aes(group=qname,colour=factor(qname)))+ theme(legend.position = c(0.8,0.7))+labs(color="Yield",y="Yield (t)") 
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_data - catch components.png"), height=4.5, width=6,dpi=600)
  
  pct.bms<-100*(stk.bms@landings/stk@catch)
  pct.ibc<-100*(stk.ibc@landings/stk@catch)
  ...
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_data - catch comp percent of catch 1.png"), height=4.5, width=6,dpi=600)
  
  ggplot(data=FLQuants(BMS=pct.bms[,ac(rec.period)],IBC=pct.ibc[,ac(rec.period)]),aes(year,data))+
    geom_line(aes(group=qname,colour=factor(qname)))+ theme(legend.position = c(0.85,0.85))+labs(color="% of catch",y="Yield (t)") 
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_data - catch comp percent of catch 2.png"), height=4.5, width=6,dpi=600)
}
...
if(doPlot){
  plot(stk)
  plotStock(stk,file=paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_Stock summary plots"))
}
...
if(doPlot){
  windows()
  plot(fit)
  savePlot(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_SAM fit - stock summary"),res=600,type="png")
}
...
if(doPlot){
  ...
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_parameter uncertainty - stock n.png"), height=4.5, width=6,dpi=600)
  ...
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_parameter uncertainty - harvest.png"), height=4.5, width=6,dpi=600)
  
  plot(stk, probs = c(0.05, 0.25, 0.5, 0.75, 0.95))
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_parameter uncertainty - stock summary.png"), height=4.5, width=6,dpi=600)
  
  rm(list=c("tmp1","tmp2","stk_coh"))
}
...
if(doPlot){
  #plot
  plot(stk_stf@m)
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_biological parameters - mortality.png"), height=4.5, width=6,dpi=600)
  plot(stk_stf@mat)
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_biological parameters - maturity.png"), height=4.5, width=6,dpi=600)
  plot(stk_stf@stock.wt)
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_biological parameters - stockwt.png"), height=4.5, width=6,dpi=600)
  plot(stk_stf@landings.wt)
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_biological parameters - landingswt.png"), height=4.5, width=6,dpi=600)
  plot(stk_stf@discards.wt)
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_biological parameters - discardswt.png"), height=4.5, width=6,dpi=600)
  plot(stk_stf@catch.wt)
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_biological parameters - catchwt.png"), height=4.5, width=6,dpi=600)
  plot(stk_stf@harvest)
  #ggplot(data=stk_stf@harvest[,ac(eqSim_yrs)], aes(age, data)) + geom_line(aes(group=year, colour=factor(year))) + 
  #  facet_wrap(~year, scales="free_y", nrow=2) + theme(legend.position = "none") + ggtitle("selectivity")
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_biological parameters - selectivity.png"), height=4.5, width=6,dpi=600)
  
  tmp<-harvest(stk[,ac(eqSim_yrs),,,,1])
...
for(i in 1:length(eqSim_yrs)){
  ...
  if (i==1){
    plot(ages,c(tmp[,ac(eqSim_yrs[i])]),main="F at age 2008:2017",type="l",col=cols[i],ylim=range(tmp),ylab="F at age")
  }else{
    lines(ages,c(tmp[,ac(eqSim_yrs[i])]),col=cols[i])
  }
}
  legend("topleft",inset=0.02,legend=c(eqSim_yrs),col=cols,lty=1)
  
  savePlot(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_biological parameters - selectivity curves"),type="png")
...
suppressWarnings(. <- capture.output(sr <- fmle(sr))) # b is Blim
### run in parallel
# library(doParallel)
# cl <- makeCluster(10)
...
if(doPlot){
  ...
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_recruitment - worm plot.png"), height=7, width=11,dpi=300)
...
if(doPlot){
plot(idx_dev[[1]],main="NSQ1")
ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_survey - NSQ1 deviations.png"), height=4.5, width=6,dpi=600)

plot(idx_dev[[2]],main="NSQ3")
ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_survey - NSQ3 deviations.png"), height=4.5, width=6,dpi=600)
}
...
if(doPlot){
  ...
  as.data.frame(FLQuants(had4_q1 = index(had_idx$NSQ1), 
                         had4_q3 = index(had_idx$NSQ3),
  ...
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_survey_TSA_sim_survey.png"), height=4.5, width=6,dpi=600)
  

  plot(idx_dev[[1]],main="NSQ1")
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_survey - NSQ1 deviations with historical corrections.png"), height=4.5, width=6,dpi=600)
  
  plot(idx_dev[[2]],main="NSQ3")
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_survey - NSQ3 deviations with historical corrections.png"), height=4.5, width=6,dpi=600)
...
if(doPlot){
  ...
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_survey_SAM_sim_survey.png"), height=4.5, width=6,dpi=600)
...
if(doPlot){
  ### compare simulated to original survey(s)
  as.data.frame(FLQuants(tsa_q1 = index(hadTSA_idx$NSQ1), 
                         tsa_q3 = index(hadTSA_idx$NSQ3),
                         sam_q1 = (index(hadSAM_idx$NSQ1)),
                         sam_q3 = (index(hadSAM_idx$NSQ3))
  )) %>%
    mutate(survey = ifelse(grepl(x = qname, pattern = "*_q1$"), "Q1", "Q3"),
           source = ifelse(grepl(x = qname, pattern = "^sam*"), "sam", "tsa")) %>%
  #  filter(year <= 2019) %>%
    ggplot(aes(x = year, y = data, colour = source)) +
    facet_grid(paste("age", age) ~ paste("IBTS", survey), scales = "free_y") +
    stat_summary(fun.y = quantile, fun.args = 0.25, geom = "line",
  ...
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_survey_SAMvTSA.png"), height=4.5, width=6,dpi=600)
...
if(doPlot){
  plot(catch_res)
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_catch - deviations.png"), height=4.5, width=6,dpi=600)
  
  plot(catch1)
  ggsave(paste0("output/had4/OM_plots/OM_",omName,"_",stkname,"_catch.png"), height=4.5, width=6,dpi=600)
}
